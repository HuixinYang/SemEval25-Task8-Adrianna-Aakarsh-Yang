...
if __name__ == "__main__":
    ...
    print(args)
    prompts = load_dataset('aakarsh-nair/semeval-2025-task-8-prompts-competition', split='dev')
    tests = load_dataset('aakarsh-nair/semeval-2025-task-8-test-cases-competition', split='dev')
    logging.info(f"Loaded {len(prompts)} prompts and {len(tests)} test cases") 
    ...
    idx_check=0 
    question = questions_dataset[idx_check]['question']
    df_id = questions_dataset[idx_check]['dataset']
    prompt_for_question = prompts[idx_check]
    tests_for_question = tests[idx_check]
    logging.info(f"Question: {question}, Dataset: {df_id}") 
    ...
    for _, prompt in enumerate(prompts):
        ...
        current_semeval_id = prompts['semeval_id']
        # find test cases for this question in tests dataset by finding all tests with this semeval_id
        tests_for_question = [test for test in tests if test['semeval_id'] == current_semeval_id] 
        logging.info(f"Prompt: id:{prompt['semeval_id']}, question:{prompt['question']}, predicted_type:{prompt['predicted_type']} found {len(tests_for_question)} test cases")
        #logging.info(f"{prompt['content']}")

        for test in tests_for_question:
            logging.info(f"Test: {test.keys()}")
